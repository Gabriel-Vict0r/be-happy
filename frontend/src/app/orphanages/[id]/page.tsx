...
async function fetchImages(id: string) { 
  ...
  export default async function PageOrphanage({ params }: any) {
    ...
    const listImage = images.filter((image: IImage, index: number) => index !== 0)
    ...
    return <main className="flex bg-bg-form flex-col">
      ...
      <div className="flex flex-col items-center gap-3 rounded-2xl bg-white w-full pb-10 md:w-[70%]">
        ...
        /> */}
        <div style={{backgroundImage: `url(${images[0].url})`}} className="w-full h-[336px] bg-cover bg-no-repeat bg-center rounded-t-2xl"></div>
        <div className="flex flex-row items-center justify-between flex-wrap gap-2">
          {listImage.map((image: IImage) => (
            <div key={image.id} style={{backgroundImage: `url(${image.url})`}} className="w-[94px] h-[94px] bg-cover bg-no-repeat bg-center rounded-xl xl:flex-grow"></div>
          ))}
        </div>
        </div>
...
<div className="flex flex-col bg-gradient-to-r from-first-gradient to-second-gradient p-4 rounded-2xl border-border-box-hour w-full border md:w-1/2">
          ...
          <div className={`flex flex-col p-4 rounded-2xl border w-full bg-gradient-to-r ${weekend.GRADIENT_START} ${weekend.GRADIENT_END} border-${weekend.BORDER_COLOR} md:w-1/2`}>
          <IoAlertCircleOutline className={`text-3xl ${weekend.COLOR_ICON}`}/>
          <p className={`font-semibold text-base ${weekend.COLOR_ICON}`}>{weekend.TEXT}</p>
          </div>
          </div>
          <Link href={`https://wa.me/55${numberOrph}?text=${encodeURIComponent(`Olá! gostaria de conhecer o Orfanato ${orph.name}.`)}`} className="flex justify-center items-center gap-2 bg-green-contact p-4 rounded-2xl text-center font-extrabold text-lg" target="_blank">
          <FaWhatsapp />
